...
export default function AIInterviewPage() {
  ...
  const handleNavigationConfirm = async () => {
    ...
    try{
      const response = await axios.delete(`http://localhost:8097/interview/deleteSessionById/${interviewDetails.current.sessionId}`,
  ...
  useEffect(() => {
    const fetchIntroQuestion = async () => {
      ...
      if (currentQuestionIndex === 0) {
        ...
        try {
          const response = await axios.post("http://localhost:8097/interview/startInterview", formData, {
            headers: {
    ...
    if (currentQuestionIndex <= interview.numberOfQuestions+1) {
      ...
      try {
       const response = await axios.post(
          `http://localhost:8097/interview/handleAnswer/${interviewDetails.current.sessionId}`,
          {
